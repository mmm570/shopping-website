<template>
  <div class="indicator" v-if="currectRouter !== 'shopCart'">
    <span
      ...
      v-if="Object.keys(shopCartProducts).length !== 0 && !isOpenDrawer"
    >
                    ...
                    <div class="flex flex-col justify-between py-3">
                      ...
                      <div>
                        <input
                          ...
                          v-model.number="shopCartProducts[key].count"
                        />
                      </div>
...
function onInput(event, key) {
  ...
  if (val === '0' || val === 0) {
    Swal.fire({
      ...
      shopCartProducts.value[key].count = 1
    })
  }
  ...
  shopCartProducts.value[key].count = Number(val)
}
</script>
